  ...
  if $ensure == 'installed' {
    ...
    if ( $use_cache ){
      ...
      exec { 'get_jdk_installer':
        ...
        command => "wget -c -T 1800 --no-cookies --no-check-certificate --header \"Cookie: gpw_e24=http%3A%2F%2Fwww.oracle.com\" --header \"Cookie: oraclelicense=accept-securebackup-cookie\" \"${javaDownloadURI}\" -O ${installerFilename}",
        timeout => 600,
